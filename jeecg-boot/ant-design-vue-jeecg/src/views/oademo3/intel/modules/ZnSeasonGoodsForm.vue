        ...
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item label="每个门店每只单品在季节内的周销量的平均值" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['avgQtyWeek']" placeholder="请输入每个门店每只单品在季节内的周销量的平均值" style="width: 100%"/>
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'shopid','goodsid','begindate','enddate','avgQtyWeek','sdate'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'shopid','goodsid','begindate','enddate','avgQtyWeek','sdate'))
      },
    }
  }
</script>